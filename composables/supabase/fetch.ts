declare global {
  type G_SYNC_STATE = "pending" | "success" | "fail";
}

const [loadingStart, loadingEnd] = exclusiveRunner(
  () => {
    useState<G_SYNC_STATE>("g_sync").value = "pending";
  },
  ({ error }) => {
    useState("g_sync").value = error ? "fail" : "success";
  }
)();
